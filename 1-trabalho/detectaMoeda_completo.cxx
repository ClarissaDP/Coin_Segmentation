...
int main(int argc, char *argv[]) {
	...
	unsigned int hue, tamanhoLinha, tamanhoColuna, quant, inicioLinha, inicioColuna, aceito, somaLinha, somaColuna, maxLinha, maxColuna;
	unsigned char val;
...
/*------------------------------------------------------------------------------*/
	...
	inicioLinha = i+5;
	MAX_LIN = inicioLinha + somaLinha;


	puts("");
	printf("Linha: %d -> TamanhoLinha: %d\n", i, tamanhoLinha);
	printf("inicioLinha %d -> finalLinha: %d\n", inicioLinha, MAX_LIN);
		...
		if(tamanhoColuna > (MAX_LIN-100)) {

			puts("Entrou na coluna invertida");
			...
			inicioColuna = colunaMaior - somaColuna;
			MAX_COL = colunaMaior;
			...
			MAX_COL = colunaMaior + somaColuna;

		}

puts("");
  printf("Coluna: %d -> TamanhoColuna: %d\n", colunaMaior, maiorDiametroColuna);
  printf("inicioColuna %d -> finalColuna: %d\n", inicioColuna, MAX_COL);
...
puts("");
  printf ("O diametro da moeda na imagem %s é de %d pixels na linha %d | e | %d pixels na coluna %d\nDiametro medio = %d\n\n", 
    argv[1], maiorDiametro, linhaMaior, maiorDiametroColuna, colunaMaior, media);	
